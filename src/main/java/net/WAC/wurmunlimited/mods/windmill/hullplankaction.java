...
public class hullplankaction implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer
{
    public static short actionId;
    static ActionEntry actionEntry;

    public hullplankaction() {
        hullplankaction.actionId = (short)ModActions.getNextActionId();
        ModActions.registerAction(hullplankaction.actionEntry = ActionEntry.createEntry(hullplankaction.actionId, "Create hull planks", "Creating hull planks", new int[0]));
    }
    ...
    public short getActionId() {
        return hullplankaction.actionId;
    }
    ...
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item target) {
        if (performer instanceof Player && target.getTemplateId() == windmill.sawmilltemplateid) {
            return Arrays.asList(hullplankaction.actionEntry);
        }
        return null;
    }
    ...
    public boolean action(final Action act, final Creature performer, final Item target, final short action, final float counter) {
        ...
        if (target.getTemplateId() == windmill.sawmilltemplateid) {
            if (counter == 1.0f) {
                ...
                performer.getCommunicator().sendNormalServerMessage("You start to create hull planks.");
            }
            if (act.currentSecond() % tickTimes == 0) {
                ...
                return windmill.sawmillItemCreate(performer, target, 546, 9, 8000, windmill.maxnums, 15000, "sound.work.carpentry.saw");
            }
        }
        return false;
    }
