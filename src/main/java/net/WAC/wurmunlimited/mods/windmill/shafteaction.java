...
public class shafteaction implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer
{
    ...
    public shafteaction() {
        shafteaction.actionId = (short)ModActions.getNextActionId();
        ModActions.registerAction(shafteaction.actionEntry = ActionEntry.createEntry(shafteaction.actionId, "Create shafts", "Creating shafts", new int[0]));
    }
    ...
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item target) {
        if (performer instanceof Player && target.getTemplateId() == windmill.sawmilltemplateid) {
            return Arrays.asList(shafteaction.actionEntry);
        }
        return null;
    }
    ...
    public boolean action(final Action act, final Creature performer, final Item target, final short action, final float counter) {
        ...
        if (target.getTemplateId() == windmill.sawmilltemplateid) {
            if (counter == 1.0f) {
                ...
                performer.getCommunicator().sendNormalServerMessage("You start to create shafts.");
            }
            if (act.currentSecond() % tickTimes == 0) {
                ...
                return windmill.sawmillItemCreate(performer, target, 23, 9, 1000, windmill.maxnums, 1000, "sound.work.carpentry.saw");
            }
        }
        return false;
    }
