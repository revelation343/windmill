...
public class slabaction implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer
{
    public static short actionId;
    static ActionEntry actionEntry;

    public slabaction() {
        slabaction.actionId = (short)ModActions.getNextActionId();
        ModActions.registerAction(slabaction.actionEntry = ActionEntry.createEntry(slabaction.actionId, "Create slabs", "Creating slabs", new int[0]));
    }
    ...
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item target) {
        if (performer instanceof Player && target.getTemplateId() == windmill.masonsmilltemplateid) {
            return Arrays.asList(slabaction.actionEntry);
        }
        return null;
    }
    ...
    public boolean action(final Action act, final Creature performer, final Item target, final short action, final float counter) {
        ...
        if (target.getTemplateId() == windmill.masonsmilltemplateid) {
            if (counter == 1.0f) {
                ...
                performer.getCommunicator().sendNormalServerMessage("You start to create slabs.");
            }
            if (act.currentSecond() % tickTimes == 0) {
                ...
                return windmill.masonsmillItemCreate(performer, target, 1, 80000, windmill.maxnums, 1000, "sound.work.mining3");
            }
        }
        return false;
    }
