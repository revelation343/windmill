...
public class woodbeamaction implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer
{
    public static short actionId;
    static ActionEntry actionEntry;

    public woodbeamaction() {
        woodbeamaction.actionId = (short)ModActions.getNextActionId();
        ModActions.registerAction(woodbeamaction.actionEntry = ActionEntry.createEntry(woodbeamaction.actionId, "Create wooden beams", "Creating wooden beams", new int[0]));
    }
    ...
    public short getActionId() {
        return woodbeamaction.actionId;
    }
    ...
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item target) {
        if (performer instanceof Player && target.getTemplateId() == windmill.sawmilltemplateid) {
            return Arrays.asList(woodbeamaction.actionEntry);
        }
        return null;
    }
    ...
    public boolean action(final Action act, final Creature performer, final Item target, final short action, final float counter) {
        ...
        if (target.getTemplateId() == windmill.sawmilltemplateid) {
            if (counter == 1.0f) {
                ...
                performer.getCommunicator().sendNormalServerMessage("You start to create wooden beams.");
            }
            if (act.currentSecond() % tickTimes == 0) {
                ...
                return windmill.sawmillItemCreate(performer, target, 860, 9, 15000, windmill.maxnums, 1000, "sound.work.carpentry.saw");
            }
        }
        return false;
    }
