...
public class plankaction implements WurmServerMod, ItemTypes, MiscConstants, ModAction, BehaviourProvider, ActionPerformer
{
    ...
    public plankaction() {
        plankaction.actionId = (short)ModActions.getNextActionId();
        ModActions.registerAction(plankaction.actionEntry = ActionEntry.createEntry(plankaction.actionId, "Create planks", "Creating planks", new int[0]));
    }
    ...
    public List<ActionEntry> getBehavioursFor(final Creature performer, final Item target) {
        if (performer instanceof Player && target.getTemplateId() == windmill.sawmilltemplateid) {
            return Arrays.asList(plankaction.actionEntry);
        }
        return null;
    }
